 ...
 *
 */
public class UpdateRecordTask extends TaskBase implements Task {
	...
	public UpdateRecordTask(Server server, UpdateMessage msg, ClientMessageProcessor processor) throws IOException {
		...
		System.out.println("Starting update record task updating version " + versionFromClient + " of record " + this.key);
		getVersions();
	}
	...
	private void getVersions() throws IOException {
		try {
			...
			for (Map.Entry<Long, ResponseInfo> entry : sentVersion.entrySet()) {
				...
				System.out.println("Send version message to " + entry.getValue().node.getNodeName());
			}
	...
	protected void sendFailureMessage(MessageStatus status, String bucketName, Key key) {
		...
		try {
			...
			UpdateMessageResponse response = new UpdateMessageResponse(clientCorrelationId, status, bucketName, key,
					Version.NONE);
			returnChannel.write(response.getBuffer());
		} catch (IOException e) {
			// At this point just log the error.
			String msg = "Read record exception sending failure return from " + server.getCluster().thisNodeName();
	...
	public void process(ServerMessage message) throws IOException {
		...
		if (message.getType() == MessageType.SVR_VERSION_RESPONSE.getCode()) {
			...
			System.out.println("Received version response " + info.version);
			// And see if we've got a quorum
			checkVersionsComplete();

		} else if (message.getType() == MessageType.SVR_WRITE_RESPONSE.getCode()) {
	...
	private void checkVersionsComplete() {
		...
		try {
			...
			Version currentVersion = doc.getVersion();
			...
			if (matches >= quorumCount) {
				...
				if (currentVersion == null) {
					// Buggered....
					sendFailureMessage(MessageStatus.READ_QUORUM_NOT_REACHED, bucketName, key);
				} else {
	...
	private void writeRecordToNodes(Version newVersion) {
		...
		try {
			...
			for (Map.Entry<Long, ResponseInfo> entry : sentWrite.entrySet()) {
				long correlationId = entry.getKey().longValue();
				ServerWriteMessage msg = new ServerWriteMessage(correlationId, bucketName, key, newVersion, payload);
				server.addTask(correlationId, this); // **before ** we actually send the message (race condition).
				entry.getValue().node.send(msg);
			}
	...
	private void checkComplete() {
		...
		if(toReceive == 0 && savedRecordCount < server.getSettings().getReplicaCount()) {
			try {
				Bucket bucket = server.getBucket(bucketName);
	...
	private void returnResponseToClient(Version newVersion) {
		complete = true;
		try {
			UpdateMessageResponse clientResponse = new UpdateMessageResponse(clientCorrelationId, MessageStatus.OK,
					bucketName, key, newVersion);
			returnChannel.write(clientResponse.getBuffer());
		} catch (IOException e) {
			// Not much we can do if there's a problem writing back to client. Most likely the
			// network connection has broken. At this point then, just log the error
			// TODO log error
		}
	}
